...
/// Moteur de simulation pur : fait avancer chaque bus le long du trajet de
/// sa ligne, arrêt après arrêt, en recalculant à chaque avancée :
///   - sa position GPS interpolée,
///   - le prochain arrêt,
///   - le temps estimé avant d'y arriver (ETA),
///   - un niveau de remplissage qui évolue occasionnellement.
///
/// Aucun timer, aucune dépendance à Riverpod : [Random], la durée de tick et
/// les lignes sont injectables, ce qui permet des tests unitaires
/// déterministes (mouvement, ETA, rebroussement aux terminus) sans dépendre
/// de l'horloge réelle ni des données du réseau réel.
class BusSimulationEngine {
  BusSimulationEngine({
    required this.tickInterval,
    List<BusLine>? lines,
    Random? random,
  })  : lines = lines ?? DakarNetwork.lines,
        _random = random ?? Random();

  /// Lignes sur lesquelles circulent les bus.
  final List<BusLine> lines;

  /// Durée entre deux avancées de la simulation.
  ...
  BusLine lineById(String id) => lines.firstWhere((l) => l.id == id);

  /// Positionne [busesPerLine] bus par ligne, répartis sur des segments et
  /// dans des sens variés pour que la simulation semble déjà "en cours" au
  /// démarrage. Enregistre au passage la vitesse de chaque bus.
  List<Bus> initializeBuses({int busesPerLine = 3}) {
    ...
    for (final line in lines) {
      for (int i = 0; i < busesPerLine; i++) {
        ...
        final nextIndex = _nextIndex(segmentIndex, forward, line);
        final position = _interpolate(
          line.stops[segmentIndex].position,
          line.stops[nextIndex].position,
          segmentProgress,
        );
        ...
        initialBuses.add(Bus(
          ...
          nextStopName: line.stops[nextIndex].name,
  ...
  /// Fait avancer chaque bus d'une durée de [tickInterval].
  List<Bus> advance(List<Bus> buses) => [for (final bus in buses) _advanceBus(bus)];

  Bus _advanceBus(Bus bus) {
    final line = lineById(bus.lineId);
    final speed = _busSpeeds[bus.id]!;
    final dtSeconds = tickInterval.inMilliseconds / 1000.0;
